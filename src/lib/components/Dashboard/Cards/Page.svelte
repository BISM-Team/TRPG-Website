<script lang="ts">
  ...
  import type { ActionData } from '../../../../routes/wikis/[wiki]/pages/[page]/$types';
  ...
  $: page_source = source
    .slice(0, source.indexOf('#') !== -1 ? source.indexOf('#') : undefined)
    .toLowerCase();
  ...
  async function fetchPage() {
    const page_response = await (fetch as typeof kit_fetch)(`/api/wikis/${wikiId}/pages/${source}`);
  ...
  const addHeading: SubmitFunction = async function ({ formData }) {
    ...
    const actual: Root = await parseSource(
      addHash(`${heading}`, level),
      (
...
<div class="content card">
  {#if page_store}
    ...
    {:then data}
      {#if !edit}
        <div class="absolute top-0 block w-full text-right shadow-sm">
          <button
            class="btn"
            on:click={toggleEdit}
            disabled={disabled || (page_store.editing && !edit)}
      ...
      <WikiPage
        ...
        {handleSave}
        {addHeading}
        saveAction="/wikis/{wikiId}/{page_source}?/update"
        {heading}
        disabled={disabled || (page_store.editing && !edit)}
        bind:edit
        result={actionResult
          ? {
              conflict:
                actionResult.creation_conflict ||
                actionResult.update_conflict ||
                actionResult.delete_conflict ||
                false,
              client_error:
                actionResult.invalid_campaign_id_or_page_name ||
                actionResult.missing_hash ||
                actionResult.missing_page ||
                actionResult.missing_text_or_tree ||
                actionResult.no_first_heading ||
                false
            }
          : null}
      />
