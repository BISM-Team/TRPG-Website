...
{#if showSaveDialog}
  <Modal {disabled} on:close={toggleSaveDialog}>
    <h3 class="w3-padding h3">Do you want to save your changes?</h3>
    <form action="?/save" method="post" use:enhance={submitSave}>
      ...
      <button {disabled} type="submit" class="btn-secondary btn" name="save" value="false"
        >Discard</button
      >
      <button {disabled} type="submit" class="btn-primary btn" name="save" value="true">Yes</button>
    </form>
  </Modal>
{/if}
