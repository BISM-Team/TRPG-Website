<script lang="ts">
  ...
  import { context, type ContextType, type WritableOf } from "$lib/components/Dashboard/context";
  ...
  const pages = writable<WritableOf<ContextType.pages>>(new Map());
...
<Create bind:dashboard={data.dashboard} dashboardId={data.params.dashboard} bind:edited bind:disabled bind:this={create}/>

<Menu   dashboard={data.dashboard} bind:disabled bind:this={menu} bind:edited  bind:edit deleteRedirectUrl={`/campaign/${data.params.campaign}/dashboards`}
